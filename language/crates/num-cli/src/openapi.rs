...
pub fn import_openapi_client(path: &Path, module_name: Option<&str>) -> Result<String, String> {
    let source = fs::read_to_string(path)
        .map_err(|err| format!("failed to read {}: {err}", path.display()))?;
    let document = parse_openapi_document(path, &source)?;
    Ok(render_openapi_client_stub(&document, module_name))
}
...
pub fn render_openapi_client_stub(document: &Value, module_name: Option<&str>) -> String {
    let module_name = module_name.unwrap_or("generated.openapi");
    let connector_name = document
        .pointer("/info/title")
        .and_then(Value::as_str)
        ...
        .unwrap_or_else(|| "openapi".to_string());
    let operations = operations(document);
    let class_name = format!("{}Client", to_type_name(&connector_name));
    ...
    out.push_str(module_name);
    out.push_str(" */\n\n");
    render_typescript_runtime_types(&mut out);
    render_typescript_component_types(&mut out, document);
    out.push_str("export class ");
    out.push_str(&class_name);
    out.push_str(" {\n");
    out.push_str("  constructor(private readonly transport: NumOpenApiTransport) {}\n\n");
    for operation in &operations {
        render_typescript_client_method(&mut out, operation);
    }
    out.push_str("}\n");
    out
}

fn render_typescript_runtime_types(out: &mut String) {
    out.push_str("export type JsonValue = null | boolean | number | string | JsonValue[] | { [key: string]: JsonValue };\n\n");
    out.push_str("export interface NumOpenApiRequest {\n");
    out.push_str("  operation: string;\n");
    out.push_str("  method: string;\n");
    out.push_str("  path: string;\n");
    out.push_str("  pathParams: Record<string, string | number | boolean>;\n");
    out.push_str("  query: Record<string, string | number | boolean | null | undefined>;\n");
    out.push_str("  body?: JsonValue;\n");
    out.push_str("}\n\n");
    out.push_str("export interface NumOpenApiTransport {\n");
    out.push_str("  request<T>(request: NumOpenApiRequest): Promise<T>;\n");
    out.push_str("}\n\n");
}

fn render_typescript_component_types(out: &mut String, document: &Value) {
    for (name, schema) in component_schemas(document) {
        match oneof_union_variants(document, name, schema) {
            ...
            None => render_typescript_object_type(out, document, name, schema),
        }
    }
}

fn render_typescript_object_type(out: &mut String, document: &Value, name: &str, schema: &Value) {
    ...
    for (field, ty) in rendered.fields {
        ...
        out.push_str(&field);
        out.push_str(": ");
        out.push_str(&typescript_type(&ty));
        out.push_str(";\n");
    }
    out.push_str("}\n\n");
}

fn render_typescript_client_method(out: &mut String, operation: &Operation) {
    out.push_str("  /** ");
    ...
    for param in &operation.params {
        if matches!(
            ...
            OpenApiParamLocation::Header
                ...
                | OpenApiParamLocation::Unknown
        ) {
            out.push_str("  // OpenAPI parameter `");
            out.push_str(&sanitize_comment_text(&param.raw_name));
            out.push_str("` is preserved in the stub signature; review transport serialization before production use.\n");
        }
    }
    out.push_str("  async ");
    out.push_str(&operation.name);
    ...
    for (index, param) in operation.params.iter().enumerate() {
        ...
        out.push_str(&param.name);
        out.push_str(": ");
        out.push_str(&typescript_type(&param.ty));
    }
    out.push_str("): Promise<");
    out.push_str(&typescript_type(&operation.result));
    out.push_str("> {\n");
    out.push_str("    return this.transport.request<");
    out.push_str(&typescript_type(&operation.result));
    out.push_str(">({\n");
    out.push_str("      operation: ");
    out.push_str(&json_string(&operation.name));
    out.push_str(",\n");
    out.push_str("      method: ");
    out.push_str(&json_string(&operation.method));
    out.push_str(",\n");
    out.push_str("      path: ");
    out.push_str(&json_string(&operation.path));
    out.push_str(",\n");
    out.push_str("      pathParams: ");
    render_param_object(out, operation, OpenApiParamLocation::Path);
    out.push_str(",\n");
    out.push_str("      query: ");
    render_param_object(out, operation, OpenApiParamLocation::Query);
    if operation
        .params
        .iter()
        .any(|param| param.location == OpenApiParamLocation::Body)
    {
...
fn render_param_object(out: &mut String, operation: &Operation, location: OpenApiParamLocation) {
    ...
    for (index, param) in params.iter().enumerate() {
        ...
        out.push_str(&json_string(&param.raw_name));
        out.push_str(": ");
        out.push_str(&param.name);
    }
    out.push_str(" }");
    ...
    for (path, path_item) in paths {
        ...
        for method in ["get", "post", "put", "patch", "delete"] {
            ...
            operations.push(Operation {
                ...
                name,
                permission,
                params: operation_params(path_item, operation),
...
fn operation_params(path_item: &Value, operation: &Value) -> Vec<OperationParam> {
    let mut params = Vec::new();

    if let Some(raw_params) = path_item.get("parameters").and_then(Value::as_array) {
        for param in raw_params {
            push_operation_param(&mut params, param);
        }
    }
        ...
        })
    {
        params.push(OperationParam {
            ...
            location: OpenApiParamLocation::Body,
        });
    }
...
fn push_operation_param(params: &mut Vec<OperationParam>, param: &Value) {
    ...
    let location = param
        .get("in")
        .and_then(Value::as_str)
        .map(openapi_param_location)
        .unwrap_or(OpenApiParamLocation::Unknown);
    let ty = param
        .get("schema")
        .map(schema_type)
        .unwrap_or_else(|| "Json".to_string());
...
fn openapi_param_location(value: &str) -> OpenApiParamLocation {
    match value {
        "path" => OpenApiParamLocation::Path,
        "query" => OpenApiParamLocation::Query,
        "header" => OpenApiParamLocation::Header,
        "cookie" => OpenApiParamLocation::Cookie,
        _ => OpenApiParamLocation::Unknown,
    }
}
...
fn typescript_type(ty: &str) -> String {
    ...
    {
        return format!("{}[]", typescript_type(inner));
    }
    if let Some(inner) = ty
        .strip_prefix("Option<")
        .and_then(|value| value.strip_suffix('>'))
    ...
    match ty {
        ...
        "Int" | "Float" | "Decimal" => "number".to_string(),
...
"##,
    ...
    #[test]
    fn renders_openapi_client_stub_fixture() {
        let document: Value = serde_json::from_str(include_str!(
            "../tests/fixtures/openapi/runtime_client.json"
        ))
        .unwrap();
        let expected = include_str!("../tests/fixtures/openapi/runtime_client.expected.ts");

        let rendered = render_openapi_client_stub(&document, Some("generated.customer"));

        assert_eq!(rendered, expected);
    }

    #[test]
    fn imports_openapi_client_stub_from_file() {
        let root = temp_dir("client_stub_import");
        let path = root.join("runtime_client.json");
        fs::write(
            &path,
            include_str!("../tests/fixtures/openapi/runtime_client.json"),
        )
        .unwrap();

        let rendered = import_openapi_client(&path, Some("generated.customer")).unwrap();

        assert!(rendered.contains("export class CustomerPortalClient"));
        assert!(rendered.contains("method: \"GET\""));
        assert!(rendered.contains("path: \"/customers/{customer_id}\""));
        assert!(rendered.contains("pathParams: { \"customer_id\": customer_id }"));
        assert!(rendered.contains("query: { \"include_inactive\": include_inactive }"));
        assert!(rendered.contains("body: body as JsonValue"));
        assert!(rendered.contains("authentication binding is not generated yet"));
        fs::remove_dir_all(root).unwrap();
    }
