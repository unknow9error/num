...
pub fn import_sql_client(path: &Path, module_name: Option<&str>) -> Result<String, String> {
    let source = fs::read_to_string(path)
        .map_err(|err| format!("failed to read {}: {err}", path.display()))?;
    Ok(render_sql_client(&source, module_name))
}
...
pub fn render_sql_client(source: &str, module_name: Option<&str>) -> String {
    let module_name = module_name.unwrap_or("generated.database");
    let tables = parse_schema(source);
    let client_name = format!("{}Client", to_type_name(&module_name.replace('.', "_")));
    let mut out = String::new();

    out.push_str("// Generated by `num import sql --client`.\n");
    out.push_str("// Boundary only: provide a NumSqlDriver for real database access.\n");
    out.push_str("// Unsupported: arbitrary SQL, updates/deletes, transactions, relation loading, migrations, and live introspection.\n\n");
    render_sql_client_prelude(&mut out);

    for table in &tables {
        render_sql_client_table(&mut out, table);
        out.push('\n');
    }

    out.push_str("export const NUM_SQL_TABLES = {\n");
    for table in &tables {
        render_sql_table_metadata(&mut out, table);
    }
    out.push_str("} as const;\n\n");

    render_recording_driver(&mut out);

    out.push_str("export class ");
    out.push_str(&client_name);
    out.push_str(" {\n");
    out.push_str("  constructor(private readonly driver: NumSqlDriver) {}\n\n");
    for table in &tables {
        render_sql_client_methods(&mut out, table);
    }
    out.push_str("}\n");
...
fn render_sql_client_prelude(out: &mut String) {
    out.push_str("export type NumSqlValue = string | number | boolean | null | Uint8Array | Record<string, unknown> | unknown[];\n\n");
    out.push_str("export interface NumSqlColumnMetadata {\n");
    out.push_str("  name: string;\n");
    out.push_str("  property: string;\n");
    out.push_str("  type: string;\n");
    out.push_str("  nullable: boolean;\n");
    out.push_str("  primaryKey: boolean;\n");
    out.push_str("}\n\n");
    out.push_str("export interface NumSqlIndexMetadata {\n");
    out.push_str("  name: string;\n");
    out.push_str("  columns: string[];\n");
    out.push_str("  unique: boolean;\n");
    out.push_str("}\n\n");
    out.push_str("export interface NumSqlForeignKeyMetadata {\n");
    out.push_str("  columns: string[];\n");
    out.push_str("  referencedTable: string;\n");
    out.push_str("  referencedColumns: string[];\n");
    out.push_str("}\n\n");
    out.push_str("export interface NumSqlTableMetadata {\n");
    out.push_str("  table: string;\n");
    out.push_str("  type: string;\n");
    out.push_str("  columns: NumSqlColumnMetadata[];\n");
    out.push_str("  primaryKey: string[];\n");
    out.push_str("  indexes: NumSqlIndexMetadata[];\n");
    out.push_str("  foreignKeys: NumSqlForeignKeyMetadata[];\n");
    out.push_str("}\n\n");
    out.push_str("export interface NumSqlListRequest {\n");
    out.push_str("  operation: string;\n");
    out.push_str("  table: string;\n");
    out.push_str("  metadata: NumSqlTableMetadata;\n");
    out.push_str("}\n\n");
    out.push_str("export interface NumSqlFindRequest {\n");
    out.push_str("  operation: string;\n");
    out.push_str("  table: string;\n");
    out.push_str("  metadata: NumSqlTableMetadata;\n");
    out.push_str("  criteria: Record<string, NumSqlValue>;\n");
    out.push_str("}\n\n");
    out.push_str("export interface NumSqlInsertRequest<TRow> {\n");
    out.push_str("  operation: string;\n");
    out.push_str("  table: string;\n");
    out.push_str("  metadata: NumSqlTableMetadata;\n");
    out.push_str("  row: TRow;\n");
    out.push_str("}\n\n");
    out.push_str("export type NumSqlRecordedRequest = NumSqlListRequest | NumSqlFindRequest | NumSqlInsertRequest<unknown>;\n\n");
    out.push_str("export interface NumSqlDriver {\n");
    out.push_str("  list<TRow>(request: NumSqlListRequest): Promise<TRow[]>;\n");
    out.push_str("  findOne<TRow>(request: NumSqlFindRequest): Promise<TRow | null>;\n");
    out.push_str("  insert<TRow>(request: NumSqlInsertRequest<TRow>): Promise<TRow>;\n");
    out.push_str("}\n\n");
}

fn render_sql_client_table(out: &mut String, table: &Table) {
    ...
    for column in &table.columns {
        ...
        out.push_str(": ");
        out.push_str(&typescript_type(&column.ty));
        out.push_str(";\n");
    }
    out.push_str("}\n");
}

fn render_sql_table_metadata(out: &mut String, table: &Table) {
    let table_ident = to_identifier(&table.name);
    out.push_str("  ");
    out.push_str(&table_ident);
    out.push_str(": {\n");
    out.push_str("    table: ");
    render_ts_string(out, &table.name);
    out.push_str(",\n");
    out.push_str("    type: ");
    render_ts_string(out, &to_type_name(&table.name));
    out.push_str(",\n");

    out.push_str("    columns: [\n");
    for column in &table.columns {
        out.push_str("      { name: ");
        render_ts_string(out, &column.name);
        out.push_str(", property: ");
        render_ts_string(out, &to_identifier(&column.name));
        out.push_str(", type: ");
        render_ts_string(out, &column.ty);
        out.push_str(", nullable: ");
        out.push_str(if column.nullable { "true" } else { "false" });
        out.push_str(", primaryKey: ");
        out.push_str(if column.primary_key { "true" } else { "false" });
        out.push_str(" },\n");
    }
    out.push_str("    ],\n");
    ...
    for index in &table.indexes {
        out.push_str("      { name: ");
        render_ts_string(out, &index.name);
        out.push_str(", columns: [");
        render_ts_string_list(out, &index.columns);
        out.push_str("], unique: ");
    ...
    for foreign_key in &table.foreign_keys {
        out.push_str("      { columns: [");
        render_ts_string_list(out, &foreign_key.columns);
        out.push_str("], referencedTable: ");
        render_ts_string(out, &foreign_key.referenced_table);
        out.push_str(", referencedColumns: [");
        render_ts_string_list(out, &foreign_key.referenced_columns);
        out.push_str("] },\n");
    }
    out.push_str("    ],\n");
    out.push_str("  } satisfies NumSqlTableMetadata,\n");
}

fn render_recording_driver(out: &mut String) {
    out.push_str("export class NumSqlRecordingDriver implements NumSqlDriver {\n");
    out.push_str("  readonly calls: NumSqlRecordedRequest[] = [];\n");
    out.push_str("  private readonly rows = new Map<string, unknown[]>();\n\n");
    out.push_str("  seed<TRow>(table: string, rows: TRow[]): void {\n");
    out.push_str(
        "    this.rows.set(table, rows.map((row) => ({ ...(row as Record<string, unknown>) })));\n",
    );
    out.push_str("  }\n\n");
    out.push_str("  async list<TRow>(request: NumSqlListRequest): Promise<TRow[]> {\n");
    out.push_str("    this.calls.push(request);\n");
    out.push_str("    return [...(this.rows.get(request.table) ?? [])] as TRow[];\n");
    out.push_str("  }\n\n");
    out.push_str("  async findOne<TRow>(request: NumSqlFindRequest): Promise<TRow | null> {\n");
    out.push_str("    this.calls.push(request);\n");
    out.push_str("    const rows = this.rows.get(request.table) ?? [];\n");
    out.push_str("    const found = rows.find((row) => Object.entries(request.criteria).every(([key, value]) => {\n");
    out.push_str("      const column = request.metadata.columns.find((candidate) => candidate.name === key);\n");
    out.push_str("      const rowObject = row as Record<string, unknown>;\n");
    out.push_str(
        "      return rowObject[column?.property ?? key] === value || rowObject[key] === value;\n",
    );
    out.push_str("    }));\n");
    out.push_str("    return (found ?? null) as TRow | null;\n");
    out.push_str("  }\n\n");
    out.push_str("  async insert<TRow>(request: NumSqlInsertRequest<TRow>): Promise<TRow> {\n");
    out.push_str("    this.calls.push(request as NumSqlInsertRequest<unknown>);\n");
    out.push_str("    const rows = this.rows.get(request.table) ?? [];\n");
    out.push_str("    rows.push({ ...(request.row as Record<string, unknown>) });\n");
    out.push_str("    this.rows.set(request.table, rows);\n");
    out.push_str("    return request.row;\n");
    out.push_str("  }\n");
    out.push_str("}\n\n");
}

fn render_sql_client_methods(out: &mut String, table: &Table) {
    let table_ident = to_identifier(&table.name);
    let table_type = to_type_name(&table.name);
    let metadata_ref = format!("NUM_SQL_TABLES.{table_ident}");
    ...
    out.push_str(&table_type);
    out.push_str("[]> {\n");
    out.push_str("    return this.driver.list<");
    out.push_str(&table_type);
    out.push_str(">({ operation: ");
    render_ts_string(out, &format!("list_{table_ident}"));
    out.push_str(", table: ");
    render_ts_string(out, &table.name);
    out.push_str(", metadata: ");
    out.push_str(&metadata_ref);
    out.push_str(" });\n");
    out.push_str("  }\n\n");

    if let Some(primary_keys) = table.primary_key_columns() {
        let finder_suffix = primary_keys
            .iter()
            .map(|column| to_identifier(&column.name))
            .collect::<Vec<_>>()
            .join("_and_");
        ...
        for (index, primary_key) in primary_keys.iter().enumerate() {
            ...
            out.push_str(&to_identifier(&primary_key.name));
            out.push_str(": ");
            out.push_str(&typescript_type(&primary_key.ty));
        }
        out.push_str("): Promise<");
        out.push_str(&table_type);
        out.push_str(" | null> {\n");
        out.push_str("    return this.driver.findOne<");
        out.push_str(&table_type);
        out.push_str(">({ operation: ");
        render_ts_string(out, &format!("find_{table_ident}_by_{finder_suffix}"));
        out.push_str(", table: ");
        render_ts_string(out, &table.name);
        out.push_str(", metadata: ");
        out.push_str(&metadata_ref);
        ...
        for (index, primary_key) in primary_keys.iter().enumerate() {
            ...
            render_ts_string(out, &primary_key.name);
            out.push_str(": ");
            out.push_str(&to_identifier(&primary_key.name));
        }
        out.push_str(" } });\n");
    ...
    out.push_str(&table_type);
    out.push_str("> {\n");
    out.push_str("    return this.driver.insert<");
    out.push_str(&table_type);
    out.push_str(">({ operation: ");
    render_ts_string(out, &format!("insert_{table_ident}"));
    out.push_str(", table: ");
    render_ts_string(out, &table.name);
    out.push_str(", metadata: ");
    out.push_str(&metadata_ref);
...
fn typescript_type(ty: &str) -> String {
    if let Some(inner) = ty
        .strip_prefix("Option<")
        .and_then(|raw| raw.strip_suffix('>'))
    {
        return format!("{} | null", typescript_type(inner));
    }

    match ty {
        "Uuid" | "Text" | "Date" | "DateTime" | "Decimal" => "string".to_string(),
...
fn render_ts_string_list(out: &mut String, values: &[String]) {
    for (index, value) in values.iter().enumerate() {
        if index > 0 {
            out.push_str(", ");
        }
        render_ts_string(out, value);
    }
}

fn render_ts_string(out: &mut String, value: &str) {
    out.push('"');
...
"#;
    ...
    #[test]
    fn renders_sql_client_boundary_fixture() {
        let source = include_str!("../tests/fixtures/sql_client/customer_portal.sql");
        let rendered = render_sql_client(source, Some("generated.db"));
        let rendered_again = render_sql_client(source, Some("generated.db"));

        assert_eq!(rendered, rendered_again);
        assert!(rendered.contains("export interface Customers"));
        assert!(rendered.contains("displayName?: string | null;"));
        assert!(rendered.contains("export interface Refunds"));
        assert!(rendered.contains("amount: string;"));
        assert!(rendered.contains("export interface NumSqlDriver"));
        assert!(rendered.contains("export class NumSqlRecordingDriver implements NumSqlDriver"));
        assert!(rendered.contains(
            "const column = request.metadata.columns.find((candidate) => candidate.name === key);"
        ));
        assert!(rendered.contains("export class GeneratedDbClient"));
        assert!(rendered.contains("list_refunds(): Promise<Refunds[]>"));
        assert!(rendered.contains(
            "find_refunds_by_tenantId_and_id(tenantId: string, id: string): Promise<Refunds | null>"
        ));
        assert!(rendered.contains("criteria: { \"tenant_id\": tenantId, \"id\": id }"));
        assert!(rendered.contains("insert_refunds(row: Refunds): Promise<Refunds>"));
        assert!(rendered.contains("name: \"idx_refunds_customer_id\""));
        assert!(rendered.contains(
            "{ columns: [\"customer_id\"], referencedTable: \"customers\", referencedColumns: [\"id\"] }"
        ));
        assert!(rendered.contains(
            "Unsupported: arbitrary SQL, updates/deletes, transactions, relation loading, migrations, and live introspection."
        ));
    }

    #[test]
    fn imports_sql_client_stub_from_file() {
        let path = Path::new("tests/fixtures/sql_client/customer_portal.sql");
        let rendered = import_sql_client(path, Some("generated.db")).unwrap();

        assert!(rendered.contains("export class GeneratedDbClient"));
        assert!(rendered.contains("NUM_SQL_TABLES.refunds"));
    }
