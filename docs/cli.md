...
num import sql --client schema.sql generated.db > src/database-client.ts
...
`num import sql --client <schema.sql> [module]` emits a TypeScript client stub
for the same generated method boundary. The stub includes table interfaces,
deterministic table metadata, `list_<table>`, primary-key
`find_<table>_by_<key>` methods, `insert_<table>`, an injected `NumSqlDriver`,
and a `NumSqlRecordingDriver` for contract tests. It preserves primary-key
column order, simple index metadata, foreign-key hints, nullable columns, and
original SQL column names in driver request criteria.
...
features such as `ALTER TABLE ... ADD CONSTRAINT`, production database adapters,
transactions, arbitrary SQL, updates/deletes, live database introspection, and
executable relation loading are not implemented yet. Migration-plan reports are
planning output only; they do not generate or execute database migration SQL.
Unsupported foreign-key and index forms are documented as outside the current
import subset rather than represented as runnable relations.
