...
- `N2704` - connector method call argument type does not match the connector
  schema.
- `N2705` - direct `fn`, `workflow`, `action`, built-in validator, or hash
  helper call has the wrong number of arguments.
- `N2706` - direct `fn`, `workflow`, `action`, built-in validator, or hash
  helper call argument type does not match the callable signature.
